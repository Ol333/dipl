...
class Example(Ui_MainWindow, QObject, Ui_Form_param, object):
    ...
    def del_proj(self):
        if self.treeView.selectedIndexes() == []:
            ...
            if d == QMessageBox.Yes:
                ...
                lgc.delete_proj(self.rwd.select_proj_by_name(
                    proj_name)[0], self.rwd)
    ...
    def open_proj(self):
        if self.treeView.selectedIndexes() == []:
            ...
            temp = self.treeView.selectionModel().model()
            proj_name = str(temp.data(self.treeView.selectedIndexes()[1]))
            out = lgc.get_safe_proj(
                self.rwd.select_proj_by_name(proj_name)[0], self.rwd)
    ...
    def error_ignore_mode(self):
        ...
        self.actionModule_skipping_on_first_error.setText(
            self.tr("Module skipping on first error"))
        self.exec_mode = 0
    ...
    def connect_slots(self):
        ...
        self.actionDelete_db_and_create_new.triggered.connect(
            self.delete_and_create_db_tables)
        ...
        self.actionExhaustive_search.triggered.connect(
            self.set_exhaustive_search)
        ...
        self.actionModule_skipping_on_first_error.triggered.connect(
            self.module_skipping_on_first_error)
            ...
            if not self.stop_fl:
                try:
                    ...
                    for k in range(len(temp_mas)):
                        temp_mas[k] = ' ' + ("\\" if temp_mas[k].startswith("#")
                                             else "") + temp_mas[k]+"  "
                    re += "\n" + "   " + \
                    ...
                    self.textEdit.append(
                        '№'+str(j)+self.tr(' launch. Error: ')+str(e))
                    self.sum_er_i.append(
                        '№'+str(j)+self.tr(' launch. Error: ')+str(e))
                    if self.exec_mode == 1:
                        d = QMessageBox.question(self.form, self.tr("Error"),
                                                 self.tr(
                                                     "Do you want continue project?"),
                                                 QMessageBox.Yes | QMessageBox.No,
                                                 QMessageBox.No)
                        if d == QMessageBox.No:
                            self.stop_project()
    ...
    def execute_with_parameters(self, i, path, module_name, sum_t):
        res_list = []
        mas, out_path = (
            self.set_and_safe_one_modul_params(i, "parameters", path))
        res_list.extend(mas)
        re = "\n" + "### " + self.tr("Module") + " №" + str(i)
        temp_txt = self.gridElementOfInput[i][3].toPlainText().split('\n')
        temp_txt = list(map(lambda x: "> "+x+'  \n', temp_txt))
        ...
        for j in range(int(self.gridElementOfInput[i][4].text())):
            if not self.stop_fl:
                try:
                    ...
                    self.moduleInfo.test_time_values(i, j, t)
                    ar.extend(
                        self.gridElementOfInput[i][3].toPlainText().split(' '))
                    temp_mas = loc_re.split('\n')
                    for k in range(len(temp_mas)):
                        temp_mas[k] = ' ' + ("\\" if temp_mas[k].startswith("#")
                                             else "") + temp_mas[k]+"  "
    ...
    def module_run_inter(self, i, set_of_modules, modules_paramValueRes, re, er_fl, sum_er, sum_t, count_of_execs):
        ...
        res_list, res, sum_t_i, out_path = self.execute_one_module(
            i, path, module_name, sum_t_i)
        modules_paramValueRes[-1][-1].extend(res_list)
        modules_paramValueRes[-1].append(out_path)  # path
        # numb
        modules_paramValueRes[-1].append(self.gridElementOfInput[i][0].text())
        # count
        modules_paramValueRes[-1].append(self.gridElementOfInput[i][4].text())
        re += res
        aver_time = self.moduleInfo.aver_time(i,
                                              int(self.gridElementOfInput[i][4].text()))

        re += self.moduleInfo.module_time(i, aver_time)
        self.timeResult.add_time_of_module(
            module_name, aver_time.total_seconds())
        ...
        if self.sum_good_i == int(self.gridElementOfInput[i][4].text()):
            self.sum_good += 1
        sum_er.append(
            list(map(lambda x: '№'+str(i)+self.tr(' module ')+x, self.sum_er_i)))
    ...
    def execute(self):
        ...
        if self.actionGenetic.text() == self.tr("Genetic +"):
            if not (er_fl and self.exec_mode == 1):
                mod_numb_launch = int(
                    self.gridElementOfInput[self.curInd][4].text())
                ...
                set_of_modules, modules_paramValueRes, re, er_fl, sum_er, sum_t, count_of_execs, module_name = self.ga.run_algorithm(
                    [0, set_of_modules, modules_paramValueRes, re, er_fl, sum_er, sum_t, all_launch])
        else:
            for i in range(len(self.gridElementOfInput)):
                if not (er_fl and self.exec_mode == 1):
                    if self.gridElementOfInput[i][2].isEnabled():
                        set_of_modules, modules_paramValueRes, re, er_fl, sum_er, sum_t, count_of_execs, module_name = self.module_run_inter(
                            i, set_of_modules, modules_paramValueRes, re, er_fl, sum_er, sum_t, count_of_execs)
    ...
    def fillParamLineEdit_ExhaustiveSearch(self, mas):
        for k in range(1 if len(mas[1][1]) == 0 else len(mas[1][1][0]['param'])+1): # перебор по одному
            ...
            for i in range(2**numbFl):  # перебор всех возможных комбинаций
                self.buttonClicked_addModule()
                self.gridElementOfInput[self.numb-1][1].setText(
                    self.gridElementOfInput[self.curInd][1].text())
                self.gridElementOfInput[self.numb-1][4].setText(
                    self.gridElementOfInput[self.curInd][4].text())
    ...
    def fillParamLineEdit_nothing(self, mas):
        for k in range(1 if len(mas[1][1]) == 0 else len(mas[1][1][0]['param'])+1): # перебор по одному
            numbFl = len(mas[1][0])
            for i in range(numbFl):  # перебор
                self.buttonClicked_addModule()
                ...
                if len(mas[1][1]) != 0:
                    ...
                    if k < len(mas[1][1][0]['param']):
                        ...
                        strings[mas[1][1][0]['number']] = temp[0] + '='
                temp = strings[mas[1][0][i]['number']].split('=', 1)
                temp[1] += mas[1][0][i]['param'] + ' '
                strings[mas[1][0][i]['number']] = '='.join(temp)
                self.gridElementOfInput[self.numb-1][3].clear()
                for i in strings:
                    self.gridElementOfInput[self.numb-1][3].append(i.strip())
        for i in range(len(self.gridElementOfInput[self.curInd])):
    ...
    def fillParamLineEdit(self):
        ...
        if mas[0][0] == 'someone want a combination':
            ...
            self.fillParamLineEdit_nothing(mas)
    ...
    def genetic_exec(self, new_str, ind_str, mst):
        ...
        for i in strings:
            self.gridElementOfInput[self.numb-1][3].append(i.strip())
        another_mas = self.module_run_inter(
            self.numb-1, mst[1], mst[2], mst[3], mst[4], mst[5], mst[6], mst[7])
    ...
    def buttonClicked_addModule(self):
        ...
        self.gridElementOfInput[self.numb][2].clicked.connect(
            self.buttonClicked_adp)
        self.gridElementOfInput[self.numb][5].clicked.connect(
            self.buttonClicked_del)
        self.gridElementOfInput[self.numb][1].textChanged.connect(
            self.textLineEditChange)
        self.gridElementOfInput[self.numb][2].setObjectName(
            "adp"+str(self.numb))
        self.gridElementOfInput[self.numb][5].setObjectName(
            "btn"+str(self.numb))
        self.gridElementOfInput[self.numb][1].setObjectName(
            "prNam"+str(self.numb))
        ...
        for i in range(6):
            self.lay.addWidget(
                self.gridElementOfInput[self.numb][i], self.numb, i, 1, 1)
        self.numb += 1
    ...
    def buttonClicked_accept(self):
        ...
        for i in range(len(self.mas)):
            ...
            if self.mas[i][2].isChecked():
                tempL = list(self.mas[i][1].text().strip().split(' '))
                k = 0
                if tempL[0].isnumeric() or (tempL[0][0] == '-' and tempL[0][1:].isnumeric()):
                    flag_do_not_combination = True
                else:
                    while k < len(tempL):
        ...
        if flag_do_not_combination:
            self.outMas.append(['', combinationIndex])
            self.outMas.append([combination, search_one_by_one])
        elif len(combination) > 0 or len(search_one_by_one) > 0:
            self.outMas.append(
                ['someone want a combination', combinationIndex])
            self.outMas.append([combination, search_one_by_one])
        else:
            self.outMas.append(['', ])
